...
export const TipTapEditor = ({ input }: any) => {
  ...
  // 🚀 FIX: Přidaná živá paměť pro seznam!
  ...
  const updateToolbar = useCallback(() => {
    ...
    // 🚀 Tady to teď bezchybně čte, na čem zrovna stojíš
  ...
  const selectStyle = { padding: "4px 8px", fontSize: "12px", border: "1px solid #e2e8f0", borderRadius: "4px", background: "#fff", cursor: "pointer", color: "#475569" };
  const btnStyle = (active: boolean) => ({ padding: "4px 8px", fontSize: "12px", fontWeight: "bold", background: active ? "#f1f5f9" : "transparent", border: "1px solid #e2e8f0", borderRadius: "4px", cursor: "pointer", color: active ? "#0f172a" : "#64748b" });

  return (
    <div style={{ border: "1px solid #e2e8f0", borderRadius: "6px", overflow: "hidden", background: "#ffffff", width: "100%", marginTop: "4px" }}>
      <style>{`
        .ProseMirror:focus { outline: none !important; }
        .ProseMirror { word-break: break-word; overflow-wrap: break-word; padding: 20px; min-height: 200px; color: #1e293b; }
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 10, display: "flex", flexWrap: "wrap", gap: "6px", alignItems: "center", padding: "8px 12px", background: "#f8fafc", borderBottom: "1px solid #e2e8f0" }}>
        ...
        {/* 🚀 Napojeno na nový stav isBulletList! */}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "4px", background: "#fff", padding: "2px 4px", borderRadius: "4px", border: "1px solid #e2e8f0" }}>
          <span style={{ fontSize: "10px", color: "#94a3b8" }}>Text</span>
          <input type="color" onInput={(e: any) => editor.chain().focus().setColor(e.target.value).run()} value={activeColor} style={{ border: "none", padding: "0", width: "16px", height: "16px", cursor: "pointer" }} />
        </div>

        <div style={{ display: "flex", alignItems: "center", gap: "4px", background: "#fff", padding: "2px 4px", borderRadius: "4px", border: "1px solid #e2e8f0" }}>
          <span style={{ fontSize: "10px", color: "#94a3b8" }}>Fixa</span>
          <input type="color" onInput={(e: any) => editor.chain().focus().setHighlight({ color: e.target.value }).run()} value={activeHighlight} style={{ border: "none", padding: "0", width: "16px", height: "16px", cursor: "pointer" }} />
          <button type="button" onClick={() => editor.chain().focus().unsetHighlight().run()} style={{ fontSize: "10px", color: "#ef4444", border: "none", background: "none", cursor: "pointer" }}>X</button>
        </div>
